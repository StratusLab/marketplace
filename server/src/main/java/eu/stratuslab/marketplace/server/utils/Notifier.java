...
import eu.stratuslab.marketplace.server.cfg.ServiceConfiguration;
import eu.stratuslab.marketplace.server.cfg.ServiceParameter;
import eu.stratuslab.marketplace.server.cfg.ServiceConfiguration.AllowedParameter;
...
public class Notifier {
    ...
    private static InternetAddress getAdminEmail(ServiceConfiguration cfg) {

        try {

            String key = AllowedParameter.ADMIN_EMAIL.name();
            ServiceParameter parameter = cfg.getParameter(key);
    ...
    private static String getMailPassword(ServiceConfiguration cfg) {
        ...
        String key = AllowedParameter.MAIL_PASSWORD.name();
        ServiceParameter parameter = cfg.getParameter(key);
        if (parameter != null) {
    ...
    private static Session createSmtpSession(ServiceConfiguration cfg) {
        ...
        try {
            ...
            String key = AllowedParameter.MAIL_SSL.name();
            ServiceParameter parameter = cfg.getParameter(key);
            String useSSL = "false";
            if (parameter != null) {
                useSSL = parameter.getValue();
            }
            ...
            // specified.
            key = AllowedParameter.MAIL_USER.name();
            parameter = cfg.getParameter(key);
            ...
            key = AllowedParameter.MAIL_DEBUG.name();
            parameter = cfg.getParameter(key);
            if (parameter != null) {
